...
# class
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
class Juego_AGL(Bidimensional_Complex_Field):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.dir = os.path.dirname(__file__)

        self.param_strs = ["μ", "δ", "ξ", "α", "nu", "f"]
        self.params = [MU, DELTA, XI, ALPHA, NU, F]

        self._title = "Anisotropic Ginzburg Landau"

    def time_step_neumann(self):
        mu, delta, xi, alpha, nu, f = self.params
        ...
        A_sig[1:-1, 1:-1] += dt/(1+1j*alpha) * \
            ((mu + nu*np.cos(2*np.pi*f*t)) * A[1:-1, 1:-1] -
